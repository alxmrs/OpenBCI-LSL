...
class OpenBCIBoard(object):
  ...
  def startStreaming(self, callback, lapse=-1):
    ...
    while self.streaming:
      ...
      callback(sample)
      if(lapse > 0 and timer() - start_time > lapse):
        self.streaming = False
  ...
  Turn streaming off and disconnect from the board
  ...
  def _read_serial_binary(self, max_bytes_to_skip=3000):
    ...
    for rep in xrange(max_bytes_to_skip):
      ...
      if self.read_state == 0:
        ...
        if (val == END_BYTE):
          # If Python 2.x, use timeit.default_timer()
          # If Python 3.x, use time.perf_counter()
...
class OpenBCISample(object):
  ...
  def __init__(self, packet_id, channel_data, aux_data, timest):
    self.id = packet_id;
  ...
  def getTime(self):
    return self.getMillis(self.aux_data)*0.001

  def getMillis(self, arr):
    return (long)((self.toLittleEndian(arr[0] & 0xFFFF) << 16)
     + self.toLittleEndian(arr[1] & 0xFFFF))

  def toLittleEndian(self, n):
