...
//------------------------------------------------------------------------------
// << PUSHBUTTON CODE >>
const int pushButton = 17; // the button is on pin D17
int pushButtonValue;       // used to hold the latest button reading
int lastPushButtonValue;   // Use to remember the last button state
...
void loop() {
  ...
  if(is_running){
      ...
      OBCI.sendChannelData(sampleCounter, true);
      sampleCounter++;
...
// STREAM DATA AND FILTER COMMANDS
      ...
      case 'n':  // turn on accelerator
        ...
        OBCI.enable_accel(RATE_25HZ); // TODO: CHANGE BACK TO "RATE_25HZ"
        break;
...
void startFromScratch(){
  ...
  OBCI.useAux = true;    // option to add user data to stream not implimented yet
  OBCI.initialize();  
