...
void OpenBCI_32::sendChannelData(byte sampleNumber, boolean getTime){
  Serial0.write(sampleNumber); // 1 byte
  ADS_writeChannelData();      // 24 bytes
  if(getTime){
    curr_time = millis();
    //auxData[0] = (unsigned byte) ((curr_time & 0x00FF0000) >> 16);
    //auxData[1] = (unsigned byte) ((curr_time & 0x0000FF00) >> 8);
    //auxData[2] = (unsigned byte) ((curr_time & 0x000000FF));

    auxData[0] = (unsigned short) ((curr_time & 0xFFFF0000) >> 16 );
    auxData[1] = (unsigned short) ((curr_time & 0x0000FFFF)) ;
    auxData[2] = (unsigned short) (0x1);
    writeAuxData();
    useAux = false;
